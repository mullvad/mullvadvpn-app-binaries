...
WireGuardNT is found in `.\x86_64-pc-windows-msvc\wireguard-nt`. A [fork](https://github.com/mullvad/wireguard-nt) is used
that fixes multihop tunnels. To update it, pull the new tag from https://git.zx2c4.com/wireguard-nt. Manually verify that
the changes are sensible, rebase the `mullvad-patches-build` branch on top of it, and follow the instructions in
`MULLVAD_BUILD.md`.

Copy the resulting files `.\dist\win10\wireguard.dll` and `.\dist\wireguard.pdb` from that repo to
