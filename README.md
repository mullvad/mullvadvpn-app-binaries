...
1. Run:
   ```
   build.bat <sha256cert_sha1_hash> <sha1cert_sha1_hash>
   ```

   `sha256cert_sha1_hash` and `sha1cert_sha1_hash` refer to the SHA1 hashes of the signing
   certificates. The certificates should be available in the certificate store. Their hashes may
   be referred to as "thumbprint". They can be viewed by running `certmgr.msc` or `certlm.msc`.
